...
class LiveCacheTester {
  ...
  async testWithCredentials() {
    ...
    if (hasSingleSite) {
      ...
      this.log('SKIP', 'Single-site credentials missing', 'Set WORDPRESS_SITE_URL, WORDPRESS_USERNAME, WORDPRESS_APP_PASSWORD');
    }

    if (hasMultiSite) {
      this.log('PASS', 'Multi-site credentials found', `Site 1: ${multiSiteCredentials.site1.url}, Site 2: ${multiSiteCredentials.site2.url}`);
      await this.testMultiSiteCaching();
    } else {
      this.log('SKIP', 'Multi-site credentials missing', 'Set WORDPRESS_SITE_URL_2, WORDPRESS_USERNAME_2, WORDPRESS_APP_PASSWORD_2 for multi-site testing');
    }
    ...
    try {
      ...
      for (let i = 0; i < testCalls.length; i++) {
        ...
        try {
          ...
          if (speedup > 2) {
            this.log('PASS', `${callName} caching effective`, `${speedup.toFixed(1)}x faster (${firstTime.toFixed(1)}ms → ${secondTime.toFixed(1)}ms)`);
          } else {
  ...
  async testMultiSiteCaching() {
    ...
    try {
      ...
      this.log('PASS', 'Both sites cached', `Site1: ${stats1Before.cache.totalSize} entries, Site2: ${stats2Before.cache.totalSize} entries`);
      ...
      if (stats1After.cache.totalSize === 0 && stats2After.cache.totalSize === stats2Before.cache.totalSize) {
        this.log('PASS', 'Multi-site cache isolation verified', `Site1 cleared (${cleared} entries), Site2 preserved (${stats2After.cache.totalSize} entries)`);
      } else {
        this.log('FAIL', 'Multi-site cache isolation failed', `Site1: ${stats1After.cache.totalSize}, Site2: ${stats2After.cache.totalSize}`);
      }
