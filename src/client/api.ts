...
export class WordPressClient implements IWordPressClient {
  ...
  private async authenticateWithBasic(): Promise<boolean> {
    const hasCredentials = this.auth.username && 
      (this.auth.method === 'app-password' ? this.auth.appPassword : this.auth.password);
   ...
   */
  async request<T = any>(method: HTTPMethod, endpoint: string, data: any = null, options: RequestOptions = {}): Promise<T> {
  ...
  private updateAverageResponseTime(duration: number): void {
    ...
    this._stats.averageResponseTime = ((this._stats.averageResponseTime * (totalSuccessful - 1)) + duration) / totalSuccessful;
  ...
  async rejectComment(id: number): Promise<WordPressComment> {
    return this.put<WordPressComment>(`comments/${id}`, { status: 'unapproved' });
  }
