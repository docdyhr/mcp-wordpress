...
export class WordPressAuth {
  ...
  private async handleAppPasswordAuth(): Promise<boolean> {
    ...
    try {
      ...
      const message = 'Application Password authentication failed. Please check your credentials and ensure the application password is valid.';
  ...
  private async handleJWTAuth(): Promise<boolean> {
    ...
    try {
      ...
      const message = 'JWT authentication failed. Please check your credentials and ensure the JWT plugin is installed and configured.';
      logger.error(message, error);
      throw new Error(message);
    }
  }
