...
describe('Cache Performance Benchmarks', () => {
  ...
  describe('Latency Benchmarks', () => {
    it('should have low cache operation latency', () => {
      ...
      console.log(`Cache operation latencies - P50: ${p50.toFixed(3)}ms, P95: ${p95.toFixed(3)}ms, P99: ${p99.toFixed(3)}ms, Max: ${max.toFixed(3)}ms`);
    ...
    it('should maintain low latency under memory pressure', () => {
      ...
      for (let i = 0; i < iterations; i++) {
        const data = {
          ...
          metadata: {
            created: Date.now(),
            tags: Array(20).fill(null).map((_, j) => `tag-${i}-${j}`)
          }
        };
  ...
  describe('Memory Efficiency Benchmarks', () => {
    it('should efficiently use memory for cached objects', () => {
      const objectSizes = [
        ...
        { name: 'medium', data: { id: 1, title: 'Test', content: 'x'.repeat(1000) } },
        { name: 'large', data: { id: 1, title: 'Test', content: 'x'.repeat(10000), meta: new Array(100).fill('data') } }
      ];
    ...
    it('should maintain stable memory usage during continuous operation', async () => {
      ...
      console.log(`Memory stability - Min: ${(minHeap / 1024 / 1024).toFixed(1)}MB, Max: ${(maxHeap / 1024 / 1024).toFixed(1)}MB, Variation: ${(heapVariation * 100).toFixed(1)}%`);
  ...
  describe('Scalability Benchmarks', () => {
    it('should scale performance with cache size', () => {
      ...
      cacheSizes.forEach(size => {
        ...
        console.log(`Cache size ${size}: Fill=${fillThroughput.toFixed(0)} ops/sec, Access=${accessThroughput.toFixed(0)} ops/sec`);
      });
